...
from .models import TCode, TEnumeration, TLien
...
def get_table_prefix(table_name: str) -> str:
    ...
    """
    try:
        return TLien.objects.values_list("lie_abs", flat=True).get(lie_table=table_name)
    except TLien.DoesNotExist:
        return ""


def generate_reference(table_name: str, pk_field: str) -> str:
    """
    Génère une référence du type CL0005.
    """

    try:
        prefix = TLien.objects.values_list("lie_abs", flat=True).get(
    ...
    if model is None:
        raise ValueError(f"Aucun modèle Django associé à la table '{table_name}'")

    last = (
        model.objects.order_by(f"-{pk_field}").values_list(pk_field, flat=True).first()
    )
...
def generate_enumeration_value(enu_code: str) -> list[dict[str, str]]:
    try:
        prefix = TEnumeration.objects.filter(enu_code=enu_code).values_list(
            "enu_nom", "enu_id"
        )
        result = []
        for p in prefix:
...
def get_dernier_code(nom_table: str, isInsert: bool):
    ...
    dernier_code = (
        TCode.objects.filter(cod_table=nom_table)
        .order_by("cod_annee", "cod_mois", "cod_id")
        .last()
    )
...
def generate_code_date(nom_table: str, isInsert: bool):
    ...
    return (
        ...
        f"{int(dernier_code['cod_mois']):02d}{int(dernier_code['cod_num']):04d}"
    )
